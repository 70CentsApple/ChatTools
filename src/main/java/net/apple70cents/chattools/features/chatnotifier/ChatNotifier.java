...
public class ChatNotifier {
    ...
    public static Text deal(Text message, MessageSignatureData signature, MessageIndicator indicator) {
    ...
    private static Text highlightAndNotify(Text text, MessageIndicator indicator) {
        ...
        if (config.matchSelfName && Pattern.compile(client.player.getName().getString(), Pattern.MULTILINE).matcher(text.getString()).find()) { // 应匹配名字 && 匹配名字
            shouldMatch = true;
        }
