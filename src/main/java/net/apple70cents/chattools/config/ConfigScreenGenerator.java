...
public class ConfigScreenGenerator {
    final static File FILE = new File(ConfigScreenGenerator.class.getClassLoader().getResource("assets/chattools/config_gui.json")
                                                                 .getFile());
        ...
        for (Object categoryInfo : (List) configGuiMap.get("content")) {
            ...
            for (Object element : (List) ((Map) categoryInfo).get("content")) {
                ...
                if ("intSlider".equals(type)) {
                    category.addEntry(ConfigScreenUtils.getEntryBuilder(eb, type, key, ((Number) ((Map) element).get("min")).intValue(),
                            ((Number) ((Map) element).get("max")).intValue()));
                    ...
                    for (Object elementInner : (List) ((Map) element).get("content")) {
                        ...
                        if ("intSlider".equals(typeInner)) {
                            sub.add(ConfigScreenUtils.getEntryBuilder(eb, typeInner, keyInner,
                                    ((Number) ((Map) elementInner).get("min")).intValue(),
                                    ((Number) ((Map) elementInner).get("max")).intValue()));
                        } else {
                            sub.add(ConfigScreenUtils.getEntryBuilder(eb, typeInner, keyInner));
                        }
                    }
